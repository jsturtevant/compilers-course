...
use crate::{ClassHierarchy, SymbolTable, SemanticError};
    ...
    pub fn infer_type(
        ...
        match expr {
            ...
            Expr::Eq(left, right) => {
                ...
                let left_is_basic = matches!(&left_type, Type::Class(name) if basic_types.contains(&name.as_str()));
            ...
            Expr::Let { bindings, body } => {
                ...
                for binding in bindings {
                    ...
                    if let Err(_) = symbol_table.add_variable(binding.name.clone(), binding.typ.clone()) {
                        symbol_table.pop_scope();
            ...
            Expr::Case { expr, branches } => {
                ...
                for (i, branch) in branches.iter().enumerate() {
                    ...
                    if let Err(_) = symbol_table.add_variable(branch.name.clone(), branch.typ.clone()) {
                        symbol_table.pop_scope();
                        return Err(SemanticError::RedefinedAttribute {
                            class: self.current_class.clone().unwrap_or_default(),
...
mod tests {
    ...
    fn test_literal_types() {
        ...
        assert_eq!(
            checker.infer_type(&Expr::Integer(42), &hierarchy, &mut symbol_table).unwrap(),
            Type::Class("Int".to_string())
        );

        assert_eq!(
            checker.infer_type(&Expr::String("hello".to_string()), &hierarchy, &mut symbol_table).unwrap(),
            Type::Class("String".to_string())
        );

        assert_eq!(
            checker.infer_type(&Expr::True, &hierarchy, &mut symbol_table).unwrap(),
    ...
    fn test_arithmetic_operations() {
        ...
        assert_eq!(
            checker.infer_type(&expr, &hierarchy, &mut symbol_table).unwrap(),
            Type::Class("Int".to_string())
        );
    }
    ...
    fn test_undefined_variable() {
        ...
        assert!(checker.infer_type(&expr, &hierarchy, &mut symbol_table).is_err());
    }
}
