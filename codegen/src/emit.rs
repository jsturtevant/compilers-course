...
impl EmitContext {
    ...
    pub fn get_label_depth(&self, label: &str) -> Option<u32> {
        self.label_depths
            .get(label)
            .map(|&depth| self.current_depth - depth - 1)
    }
}
...
pub fn emit_module(program: &LirProgram, hir: &ir::hir::HirProgram) -> Result<Vec<u8>, String> {
    ...
    let user_vtable_entries: usize = program.vtables.iter().map(|v| v.methods.len()).sum();
    ...
    let runtime = add_runtime(
        &mut module,
        heap_start,
        class_name_table_addr,
        string_vtable_addr,
    );
    ...
    let builtin_vtables = vec![
        (
            "Object",
            vec!["Object_abort", "Object_copy", "Object_type_name"],
        ),
        (
            "IO",
            vec![
...
fn emit_function(ctx: &EmitContext, lir_func: &LirFunction) -> Result<Function, String> {
    ...
    let local_types: Vec<(u32, ValType)> = lir_func
        .locals
        .iter()
