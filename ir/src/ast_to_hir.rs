...
use parser::ast::{self, Class, Expr, Feature, Program};
use semant::{class_hierarchy::ClassHierarchy, SemanticAnalyzer};
use std::collections::HashMap;

/// Lower a program from AST to HIR
pub fn lower_program(program: &Program, analyzer: &SemanticAnalyzer) -> Result<HirProgram, String> {
...
impl<'a> Lowerer<'a> {
    ...
    fn build_vtable_for_builtin(&mut self, class_name: &str) {
        ...
        for (method_name, _sig) in methods {
            ...
            self.method_owners
                .insert((class_name.to_string(), method_name), defining_class);
        }
    ...
    fn lower_class(&mut self, class: &Class) -> Result<HirClass, String> {
        ...
        for feature in &class.features {
            if let Feature::Attribute(attr) = feature {
                self.var_types
                    .insert(attr.name.clone(), attr.attr_type.clone());
            }
        }
    ...
    fn lower_method(
        &mut self,
        method: &ast::MethodFeature,
        vtable_index: usize,
    ) -> Result<HirMethod, String> {
    ...
    fn lower_expr(&mut self, expr: &Expr) -> Result<HirExpr, String> {
        match expr {
            ...
            Expr::Plus(left, right) => Ok(HirExpr::Add {
                left: Box::new(self.lower_expr(left)?),
                right: Box::new(self.lower_expr(right)?),
                typ: TypeId::Int,
            }),

            Expr::Minus(left, right) => Ok(HirExpr::Sub {
                left: Box::new(self.lower_expr(left)?),
                right: Box::new(self.lower_expr(right)?),
                typ: TypeId::Int,
            }),

            Expr::Times(left, right) => Ok(HirExpr::Mul {
                left: Box::new(self.lower_expr(left)?),
                right: Box::new(self.lower_expr(right)?),
                typ: TypeId::Int,
            }),

            Expr::Divide(left, right) => Ok(HirExpr::Div {
                left: Box::new(self.lower_expr(left)?),
                right: Box::new(self.lower_expr(right)?),
                typ: TypeId::Int,
            }),
            ...
            Expr::Lt(left, right) => Ok(HirExpr::Lt {
                left: Box::new(self.lower_expr(left)?),
                right: Box::new(self.lower_expr(right)?),
                typ: TypeId::Bool,
            }),

            Expr::Le(left, right) => Ok(HirExpr::Le {
                left: Box::new(self.lower_expr(left)?),
                right: Box::new(self.lower_expr(right)?),
                typ: TypeId::Bool,
            }),

            Expr::Eq(left, right) => Ok(HirExpr::Eq {
                left: Box::new(self.lower_expr(left)?),
                right: Box::new(self.lower_expr(right)?),
                typ: TypeId::Bool,
            }),
            ...
            // Unary operations
            Expr::Not(expr) => Ok(HirExpr::Not {
                expr: Box::new(self.lower_expr(expr)?),
                typ: TypeId::Bool,
            }),

            Expr::Negate(expr) => Ok(HirExpr::Negate {
                expr: Box::new(self.lower_expr(expr)?),
                typ: TypeId::Int,
            }),

            Expr::IsVoid(expr) => Ok(HirExpr::IsVoid {
                expr: Box::new(self.lower_expr(expr)?),
                typ: TypeId::Bool,
            }),
            ...
            Expr::Let { bindings, body } => {
                ...
                for binding in bindings {
                    ...
                    self.var_types
                        .insert(binding.name.clone(), binding.typ.clone());
                }
                ...
                for binding in bindings.iter().rev() {
                    ...
                    let init = if let Some(ref init_expr) = binding.init {
                        ...
                        Box::new(match typ {
                            TypeId::Int => HirExpr::IntLiteral {
                                value: 0,
                                typ: TypeId::Int,
                            },
                            TypeId::Bool => HirExpr::BoolLiteral {
                                value: false,
                                typ: TypeId::Bool,
                            },
                            TypeId::String => HirExpr::StringLiteral {
                                value: String::new(),
                                typ: TypeId::String,
                            },
                            _ => HirExpr::Object {
            ...
            Expr::Case { expr, branches } => {
                ...
                for branch in branches {
                    ...
                    self.var_types
                        .insert(branch.name.clone(), branch.typ.clone());
            ...
            Expr::FuncCall { name, args } => {
                ...
                let class_name = current_class_copy
                    .clone()
                    .unwrap_or_else(|| "Object".to_string());
...
mod tests {
    use super::*;
    use parser::ast::{Class, Expr, Feature, MethodFeature, Program};
